...
// Commenting out or removing AppointmentRecord as it's no longer directly used
...
const BookAppointment = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endDateTime = new Date(startDateTime.getTime() + 60 * 60 * 1000); // Add 1 hour for duration

      const startDateTimeISO = startDateTime.toISOString();
      const endDateTimeISO = endDateTime.toISOString();

      // Construct a summary. Include client name and service. Notes can be added if the Edge function is modified to include them in the description.
      const eventSummary = `Consultation: ${formData.serviceName} for ${formData.clientName}${formData.notes ? ' - Notes: ' + formData.notes : ''}`;

      // Add a toast message for processing
      toast({
        title: "Creating Calendar Event...",
        description: "Please wait while we schedule your appointment on the calendar.",
      });

      // Invoke the Edge Function
      const { data: functionData, error: functionError } = await supabase.functions.invoke(
        'create-google-calendar-event', // Confirmed function name
        {
          body: {
      ...
      if (functionError) {
        ...
        // The new simple function returns { error: googleErrorDetails } if Google API fails
        // but supabase.functions.invoke wraps this in its own error structure.
        // We'll rely on functionError.message for now, which might contain context like "Request failed with status 500"
        // or "Relay Error: ..." if the function itself threw an error before Google API call.
        // If functionData comes back with an error property (as per the simple function's direct Google error return)
        // we might be able to use that, but functionError is the primary Supabase client error.
        if (functionData && functionData.error) {
           // This case might not be hit if functionError is already populated due to non-2xx status
           errorMessage = `Google API Error: ${typeof functionData.error === 'object' ? JSON.stringify(functionData.error) : functionData.error}`;
        } else if (functionError.context && functionError.context.details) {
          // Supabase often wraps errors, let's try to get more specific
          errorMessage = functionError.context.details;
        }

        toast({
          title: "Calendar Event Creation Failed",
          description: `There was an issue creating the Google Calendar event. Please try again or contact us. Details: ${errorMessage}`,
          variant: "destructive",
          duration: 12000,
        });
      } else {
        ...
        toast({
          title: "Appointment Scheduled in Calendar!",
          // The simple function returns the Google event object directly as 'data'
          description: "Your consultation has been directly added to the calendar. Event ID: " + (functionData?.id || 'N/A'),
          duration: 10000,
        });
        // Reset form
        setFormData({
